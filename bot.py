"""Точка входа: инициализация бота, БД, FSM, APScheduler, Mini App.
Поддерживает SQLite (локально) и PostgreSQL (продакшен через DATABASE_URL).
"""
import asyncio
import logging
import os
...
# FSM storage: PostgreSQL в проде, SQLite локально
...
def _collect_db_helpers() -> dict:
    ...
    return {
        ...
        "get_shared_habits": db.get_shared_habits,
        "get_shared_streak": db.get_shared_streak,
        "get_shared_today_status": db.get_shared_today_status,
        "create_shared_habit": db.create_shared_habit,
        "delete_shared_habit": db.delete_shared_habit,
        "send_friend_request": db.send_friend_request,
        "accept_friend_request": db.accept_friend_request,
        "get_pending_friend_requests": db.get_pending_friend_requests,
        "get_last_activity": db.get_last_activity,
        "get_friends": db.get_friends,
        "get_user_by_username": db.get_user_by_username,
        "get_user_achievements": db.get_user_achievements,
        "get_or_create_today_quest": db.get_or_create_today_quest,
        "get_freezes_this_week": db.get_freezes_this_week,
        "can_freeze": db.can_freeze,
        "bot": bot,
