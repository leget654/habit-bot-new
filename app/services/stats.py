...
async def generate_insights(user_id: int) -> list[str]:
    ...
    async with aiosqlite.connect(db.DB_PATH) as conn:
        async with conn.execute(
            ...
            "JOIN habits h ON c.habit_id = h.id WHERE h.user_id=?",
            (user_id,)
        ) as cur:
...
async def export_csv(user_id: int) -> str:
    ...
    async with aiosqlite.connect(db.DB_PATH) as conn:
        ...
        async with conn.execute(
            "SELECT habit_id, completed_date, note FROM completions WHERE user_id=? ORDER BY completed_date DESC",
            (user_id,)
        ) as cur:
