...
from ..states import SetGoal, RenameHabit, SetTargetMinutes
...
def register_handlers(dp: Dispatcher, bot: Bot):
    ...
    async def cb_newgoal(cb: CallbackQuery, state: FSMContext):
        ...
        async with aiosqlite.connect(db.DB_PATH) as conn:
            await conn.execute("UPDATE habits SET monthly_goal=? WHERE id=?", (goal, data["habit_id"]))
            await conn.commit()
        await state.clear()
    ...
    async def fsm_rename(msg: Message, state: FSMContext):
        ...
        import aiosqlite
        async with aiosqlite.connect(db.DB_PATH) as conn:
            await conn.execute("UPDATE habits SET name=? WHERE id=?", (msg.text.strip(), data["habit_id"]))
            await conn.commit()
        await state.clear()
    ...
    async def cb_dopause(cb: CallbackQuery):
        ...
        import aiosqlite
        async with aiosqlite.connect(db.DB_PATH) as conn:
            await conn.execute("UPDATE habits SET is_paused=1 WHERE id=?", (habit_id,))
            await conn.commit()
        h = await db.get_habit(habit_id)
    ...
    async def cb_dounpause(cb: CallbackQuery):
        ...
        import aiosqlite
        async with aiosqlite.connect(db.DB_PATH) as conn:
            await conn.execute("UPDATE habits SET is_paused=0 WHERE id=?", (habit_id,))
            await conn.commit()
        h = await db.get_habit(habit_id)
    ...
    async def cb_stackset(cb: CallbackQuery):
        ...
        import aiosqlite
        async with aiosqlite.connect(db.DB_PATH) as conn:
            await conn.execute("UPDATE habits SET parent_habit_id=? WHERE id=?", (parent_id, habit_id))
            await conn.commit()
        h = await db.get_habit(habit_id)
    ...
    async def cb_stackunlink(cb: CallbackQuery):
        ...
        import aiosqlite
        async with aiosqlite.connect(db.DB_PATH) as conn:
            await conn.execute("UPDATE habits SET parent_habit_id=NULL WHERE id=?", (habit_id,))
            await conn.commit()
        h = await db.get_habit(habit_id)
